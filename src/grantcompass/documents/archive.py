...
from grantcompass.documents.archive_paths import canonical_path_key, validate_archive_path
from grantcompass.documents.archive_zip import validate_raw_archive_names
from grantcompass.documents.base import DocumentParseError, ParseErrorCode, parse_failure
...
ARCHIVE_TOO_LARGE: Final[ParseErrorCode] = "archive_too_large"
MISSING_CONTENT: Final[ParseErrorCode] = "missing_content"
...
def read_sections(content: bytes) -> tuple[SectionXml, ...]:
    ...
    try:
        validate_raw_archive_names(content)
...
def _validate_infos(infos: list[ZipInfo]) -> None:
    ...
    for info in infos:
        ...
        normalized = canonical_path_key(info.filename)
